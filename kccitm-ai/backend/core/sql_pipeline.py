...
SQL: SELECT ROUND(passed.cnt * 100.0 / total.cnt, 2) AS pass_all_pct, passed.cnt AS passed, total.cnt AS total FROM (SELECT COUNT(DISTINCT s.roll_no) AS cnt FROM students s JOIN semester_results sr ON s.roll_no = sr.roll_no GROUP BY s.roll_no HAVING SUM(CASE WHEN sr.result_status NOT IN ('PASS','PCP','PWG') AND sr.result_status NOT LIKE 'CP(%% 0)' AND sr.result_status != 'CP(0)' THEN 1 ELSE 0 END) = 0) passed, (SELECT COUNT(DISTINCT roll_no) AS cnt FROM students) total LIMIT 1

Q: "what percentage of female students passed all semesters"
SQL: SELECT ROUND(passed.cnt * 100.0 / total.cnt, 2) AS pass_pct, passed.cnt AS passed, total.cnt AS total FROM (SELECT COUNT(DISTINCT s.roll_no) AS cnt FROM students s JOIN semester_results sr ON s.roll_no = sr.roll_no WHERE s.gender = 'F' GROUP BY s.roll_no HAVING SUM(CASE WHEN sr.result_status NOT IN ('PASS','PCP','PWG') AND sr.result_status NOT LIKE 'CP(%% 0)' AND sr.result_status != 'CP(0)' THEN 1 ELSE 0 END) = 0) passed, (SELECT COUNT(DISTINCT roll_no) AS cnt FROM students WHERE gender = 'F') total LIMIT 1
